...
class AmplGraph:
    ...
    def graph_resource(self, ampl_collector=None, plot=True):
        ...
        return df_to_plot
    def graph_total_trans_cost(self):
        pio.renderers.default = 'browser'
        ...
        c_inv = results.drop(columns=['C_op', 'C_maint']).dropna(how='all')
        c_op = results.drop(columns=['C_inv', 'C_maint']).dropna(how='all')
        c_maint = results.drop(columns=['C_op', 'C_inv']).dropna(how='all')

        # Création des DataFrames de coût
        ...
        for y in c_inv.index.get_level_values(0).unique():
            temp_inv = self._remove_low_values(c_inv.loc[c_inv.index.get_level_values('Years') == y, 'C_inv'],
                                               threshold=0)
            df_to_plot_inv.update(temp_inv)

            temp_op = self._remove_low_values(c_op.loc[c_op.index.get_level_values('Years') == y, 'C_op'], threshold=0)
            df_to_plot_op.update(temp_op)

            temp_maint = self._remove_low_values(c_maint.loc[c_maint.index.get_level_values('Years') == y, 'C_maint'],
                                                 threshold=0)
            df_to_plot_maint.update(temp_maint)

        # Formatage
        def format_df(df, colname, cost_type):
            df.dropna(how='all', inplace=True)
            df.reset_index(inplace=True)
            df['Type'] = cost_type
            df.rename(columns={colname: 'Cost'}, inplace=True)
            df['Category'] = df['Elements'].replace(category)
            return df

        df_to_plot_inv = format_df(df_to_plot_inv, 'C_inv', 'C_inv')
        df_to_plot_op = format_df(df_to_plot_op, 'C_op', 'C_op')
        df_to_plot_maint = format_df(df_to_plot_maint, 'C_maint', 'C_maint')

        # Concaténation
        df_to_plot = pd.concat([df_to_plot_inv, df_to_plot_op, df_to_plot_maint], axis=0)
        df_to_plot['Years'] = df_to_plot['Years'].str.replace('YEAR_', '')

        # Ajustement électricité RE/NRE
        re_share_elec = self.re_share_elec
        cost_elec_re, cost_elec_nre = pd.DataFrame(), pd.DataFrame()
        for i, y in enumerate(['2025', '2030', '2035', '2040', '2045', '2050']):
            total_elec_cost = df_to_plot[(df_to_plot['Years'] == y) & (df_to_plot['Elements'] == 'ELECTRICITY')]['Cost']
            if not total_elec_cost.empty:
                temp_re = pd.DataFrame({
                    'Years': y,
                    'Elements': 'ELECTRICITY',
                    'Cost': re_share_elec[i] * total_elec_cost.values[0],
                    'Type': 'C_op',
                    'Category': 'RE_FUELS'
                }, index=[0])
                temp_nre = pd.DataFrame({
                    'Years': y,
                    'Elements': 'ELECTRICITY',
                    'Cost': total_elec_cost.values[0] * (1 - re_share_elec[i]),
                    'Type': 'C_op',
                    'Category': 'NRE_FUELS'
                }, index=[0])
                cost_elec_re = pd.concat([cost_elec_re, temp_re])
                cost_elec_nre = pd.concat([cost_elec_nre, temp_nre])

        # Remplace l'ancien coût NRE et ajoute RE
        df_to_plot.set_index(['Years', 'Elements', 'Category'], inplace=True)
        cost_elec_nre.set_index(['Years', 'Elements', 'Category'], inplace=True)
        df_to_plot.update(cost_elec_nre)
        df_to_plot.reset_index(inplace=True)
        df_to_plot = pd.concat([df_to_plot, cost_elec_re], axis=0)

        # Regroupement et cumul
        df_to_plot = df_to_plot.groupby(['Years', 'Category']).sum().reset_index()
        df_to_plot['Years'] = df_to_plot['Years'].astype(int)
        df_to_plot.sort_values(by='Years', inplace=True)
        df_to_plot['Cost'] = df_to_plot['Cost'] / 1000  # En b€
        df_to_plot['Cum_Cost'] = df_to_plot.groupby('Category')['Cost'].cumsum()

        # Préparation pour graphe
        order_entry = [
        ...
        order_entry = [x for x in order_entry if x in df_to_plot['Category'].unique()]
        df_to_plot['Category'] = pd.Categorical(df_to_plot['Category'], order_entry)
        df_to_plot.sort_values(by='Category', inplace=True)
        ...
        fig = px.area(
            ...
            y='Cum_Cost',
            color='Category',
            title=self.case_study + ' - Cumulative System Transition Cost [b€<sub>2015</sub>]',
            color_discrete_map=self.color_dict_full
        )
        fig.update_traces(mode='none', fill='tonexty')
        fig.update_xaxes(categoryorder='array', categoryarray=sorted(df_to_plot['Years'].unique()))
        pio.show(fig)

        if not os.path.exists(Path(self.outdir + "_Raw/")):
            Path(self.outdir + "_Raw").mkdir(parents=True, exist_ok=True)
        fig.write_html(self.outdir + "_Raw/Cum_System_cost.html")

        # Valeurs pour axe Y
        temp = df_to_plot.groupby('Years').sum()
        yvals = sorted([0, round(temp['Cum_Cost'].min(), 1), round(temp['Cum_Cost'].max(), 1)])
        title = "<b>Cumulative system transition cost</b><br>[b€<sub>2015</sub>]"

        self.custom_fig(fig, title, yvals)
        fig.write_image(self.outdir + "Cum_System_cost.pdf", width=1200, height=550)
        plt.close()
    ...
    def graph_cost_return(self, ampl_collector=None, plot=True):
        ...
        if plot:
            ...
            ELEC_2035 = float(
                df_to_plot_eff.loc[(df_to_plot_eff['Years'] == '2035') & (df_to_plot_eff['Category'] == 'ELECTRICITY')][
                    'Cost_return'])
            INF_2035 = float(df_to_plot_eff.loc[(df_to_plot_eff['Years'] == '2035') & (
                        df_to_plot_eff['Category'] == 'INFRASTRUCTURE')]['Cost_return'])
            ...
            yvals = sorted(
                [0, round(INF_2035, 1), round(ELEC_2035 + INF_2035, 1), round(temp[-1], 1), round(temp[0], 1)])
            self.custom_fig(fig, title, yvals, xvals=df_to_plot_eff['Years'].unique())
        ...
        if type_of_graph in ['C_op_phase', 'C_inv_phase_tech']:
            ...
            for sector in df_to_plot.keys():
                ...
                if len(df_to_plot[sector].index.get_level_values(0).unique()) >= 1:
                    ...
                    fig.write_image(self.outdir + "Load_factor_scaled/Diff_PF/" + sector + ".pdf", width=1200,
                                    height=550)
                plt.close()
    ...
    def dict_color(category):
        ...
        if category == 'Electricity':
            color_dict = {"NUCLEAR": "deeppink", "NUCLEAR_SMR": "deeppink", "CCGT": "darkorange",
                          "CCGT_AMMONIA": "slateblue", "COAL_US": "black", "COAL_IGCC": "dimgray", "PV": "yellow",
                          "WIND_ONSHORE": "lawngreen", "WIND_OFFSHORE": "green", "HYDRO_RIVER": "blue",
                          "GEOTHERMAL": "firebrick", "ELECTRICITY": "dodgerblue"}
        elif category == 'Heat_low_T':
            color_dict = {"DHN_HP_ELEC": "blue", "DHN_COGEN_GAS": "orange", "DHN_COGEN_WOOD": "sandybrown",
                          ...
                          "DEC_ADVCOGEN_GAS": "burlywood", "DEC_ADVCOGEN_H2": "violet", "DEC_BOILER_GAS": "moccasin",
                          "DEC_BOILER_WOOD": "peru", "DEC_BOILER_OIL": "darkorchid", "DEC_SOLAR": "yellow",
                          "DEC_DIRECT_ELEC": "deepskyblue"}
        elif category == 'Heat_high_T':
            color_dict = {"IND_COGEN_GAS": "orange", "IND_COGEN_WOOD": "peru", "IND_COGEN_WASTE": "olive",
            ...
            color_dict = {"TRAMWAY_TROLLEY": "dodgerblue", "BUS_COACH_DIESEL": "dimgrey", "BUS_COACH_HYDIESEL": "gray",
                          "BUS_COACH_CNG_STOICH": "orange", "BUS_COACH_FC_HYBRIDH2": "violet", "TRAIN_PUB": "blue",
                          "CAR_GASOLINE": "black", "CAR_DIESEL": "lightgray", "CAR_NG": "moccasin",
                          "CAR_METHANOL": "orchid", "CAR_HEV": "salmon", "CAR_PHEV": "lightsalmon",
                          "CAR_BEV": "deepskyblue", "CAR_FUEL_CELL": "magenta"}
        elif category == 'Freight':
            color_dict = {"TRAIN_FREIGHT": "royalblue", "BOAT_FREIGHT_DIESEL": "dimgrey",
                          "BOAT_FREIGHT_NG": "darkorange", "BOAT_FREIGHT_METHANOL": "fuchsia",
                          "TRUCK_DIESEL": "darkgrey", "TRUCK_FUEL_CELL": "violet", "TRUCK_ELEC": "dodgerblue",
                          "TRUCK_NG": "moccasin", "TRUCK_METHANOL": "orchid"}
        elif category == 'Ammonia':
            color_dict = {"HABER_BOSCH": "tomato", "AMMONIA": "slateblue", "AMMONIA_RE": "blue"}
        elif category == 'Methanol':
            color_dict = {"SYN_METHANOLATION": "violet", "METHANE_TO_METHANOL": "orange", "BIOMASS_TO_METHANOL": "peru",
                          "METHANOL": "orchid", "METHANOL_RE": "mediumvioletred"}
        elif category == "HVC":
            color_dict = {"OIL_TO_HVC": "blueviolet", "GAS_TO_HVC": "orange", "BIOMASS_TO_HVC": "peru",
                          "METHANOL_TO_HVC": "orchid"}
        elif category == 'Conversion':
            color_dict = {"H2_ELECTROLYSIS": "violet", "H2_NG": "magenta", "H2_BIOMASS": "orchid",
                          "GASIFICATION_SNG": "orange", "PYROLYSIS": "blueviolet", "ATM_CCS": "black",
                          "INDUSTRY_CCS": "grey", "SYN_METHANOLATION": "mediumpurple", "SYN_METHANATION": "moccasin",
                          "BIOMETHANATION": "darkorange", "BIO_HYDROLYSIS": "gold",
                          "METHANE_TO_METHANOL": "darkmagenta", 'SMR': 'orange'}
        elif category == 'Storage':
            color_dict = {"TS_DHN_SEASONAL": "indianred", "BATT_LI": "royalblue", "BEV_BATT": "deepskyblue",
                          "PHEV_BATT": "lightskyblue", "PHS": "dodgerblue", "TS_DEC_HP_ELEC": "blue",
                          "TS_DHN_DAILY": "lightcoral", "TS_HIGH_TEMP": "red", "SEASONAL_NG": "orange",
                          "SEASONAL_H2": "violet", "SLF_STO": "blueviolet", "TS_DEC_DIRECT_ELEC": "darkgoldenrod",
                          "TS_DEC_THHP_GAS": "orange", "TS_DEC_COGEN_GAS": "coral", "TS_DEC_COGEN_OIL": "darkviolet",
                          "TS_DEC_ADVCOGEN_GAS": "sandybrown", "TS_DEC_ADVCOGEN_H2": "plum", "TS_DEC_BOILER_GAS": "tan",
                          "TS_DEC_BOILER_WOOD": "peru", "TS_DEC_BOILER_OIL": "darkviolet", "GAS_STORAGE": "orange",
                          "H2_STORAGE": "violet", "CO2_STORAGE": "lightgray", "GASOLINE_STORAGE": "gray",
                          "DIESEL_STORAGE": "silver", "AMMONIA_STORAGE": "slateblue", "LFO_STORAGE": "darkviolet"}
        elif category == 'Storage_daily':
            color_dict = {"BATT_LI": "royalblue", "BEV_BATT": "deepskyblue", "PHEV_BATT": "lightskyblue",
                          "TS_DEC_HP_ELEC": "blue", "TS_DHN_DAILY": "lightcoral", "TS_HIGH_TEMP": "red",
                          "TS_DEC_DIRECT_ELEC": "darkgoldenrod", "TS_DEC_THHP_GAS": "orange",
                          "TS_DEC_COGEN_GAS": "coral", "TS_DEC_COGEN_OIL": "darkviolet",
                          "TS_DEC_ADVCOGEN_GAS": "sandybrown", "TS_DEC_ADVCOGEN_H2": "plum", "TS_DEC_BOILER_GAS": "tan",
                          "TS_DEC_BOILER_WOOD": "peru", "TS_DEC_BOILER_OIL": "darkviolet"}
        elif category == 'Resources':
            color_dict = {"ELECTRICITY": "deepskyblue", "GASOLINE": "gray", "DIESEL": "silver",
                          "BIOETHANOL": "mediumorchid", "BIODIESEL": "mediumpurple", "LFO": "darkviolet",
                          "GAS": "orange", "GAS_RE": "gold", "WOOD": "saddlebrown", "WET_BIOMASS": "seagreen",
                          "COAL": "black", "URANIUM": "deeppink", "WASTE": "olive", "H2": "violet", "H2_RE": "plum",
                          "AMMONIA": "slateblue", "AMMONIA_RE": "blue", "METHANOL": "orchid",
                          "METHANOL_RE": "mediumvioletred", "CO2_EMISSIONS": "gainsboro", "RES_WIND": "limegreen",
                          "RES_SOLAR": "yellow", "RES_HYDRO": "blue", "RES_GEO": "firebrick",
                          "ELEC_EXPORT": "chartreuse", "CO2_ATM": "dimgray", "CO2_INDUSTRY": "darkgrey",
                          "CO2_CAPTURED": "lightslategrey", "RE_FUELS": 'green', 'NRE_FUELS': 'black',
                          'LOCAL_RE': 'limegreen', 'IMPORTED_ELECTRICITY': 'deepskyblue'}
            ...
            color_dict = {"ELECTRICITY": "deepskyblue", "HEAT_HIGH_T": "red", "HEAT_LOW_T_DECEN": "lightpink",
                          ...
                          "MOBILITY_FREIGHT": "darkgoldenrod", "NON_ENERGY": "darkviolet", "INFRASTRUCTURE": "grey",
                          "HVC": "cyan", 'STORAGE': 'chartreuse', 'OTHERS': 'gainsboro'}
        elif category == 'Infrastructure':
            color_dict = {'EFFICIENCY': 'lime', 'DHN': 'orange', 'GRID': 'gold'}
        elif category == 'Years_1':
            color_dict = {'YEAR_2020': 'blue', 'YEAR_2025': 'orange', 'YEAR_2030': 'green', 'YEAR_2035': 'red',
                          'YEAR_2040': 'purple', 'YEAR_2045': 'brown', 'YEAR_2050': 'pink'}
        elif category == 'Years_2':
            color_dict = {'2020': 'blue', '2025': 'orange', '2030': 'green', '2035': 'red', '2040': 'purple',
                          '2045': 'brown', '2050': 'pink'}
        elif category == 'Phases':
            color_dict = {'2015_2020': 'blue', '2020_2025': 'orange', '2025_2030': 'green', '2030_2035': 'red',
                          '2035_2040': 'purple', '2040_2045': 'brown', '2045_2050': 'pink'}
    ...
    def custom_fig(fig, title, yvals, xvals=['2025', '2030', '2035', '2040',
        ...
        if type_graph in ['bar', 'scatter']:
            fig.update_layout(
                {ax: {"visible": False, "matches": None} for ax in fig.to_dict()["layout"] if "xaxis" in ax})
